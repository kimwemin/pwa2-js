...
// `?.`을 기준으로 앞의 평가 대상이 undefined || null 이 아니면 처리를 계속 이어가고
...
// 여러 종류의 데이터 타입을 저장 가능하나 일반적으로 하나의 데이터 타입만 저장한다.
// 배열의 길이는 동적으로 변화
...
// indexOf(searchElement) : 배열에서 특정 요소를 검색해서 인덱스로 반환
...
// 원래 값은 resultLength 값으로 그 자리에는 push로 추가한 값이 들어간다
// 원본이 바뀐다
...
// 제거한 요소를 저장해뒀다가 나중에 사용할 것이라면 상수 선언, 저장하지 않을거라면 생략 후 작성
const arr6 = [1, 2, 3];
...
// (a - b)가 양수일 경우, a가 큰 수, b가 작은 수로 인식해서 정렬
// (a - b)가 음수일 경우, a가 작은 수, b가 큰 수로 인식해서 정렬
...
const resultMapTng = arr15.map(val => {
  if(val % 2 === 0) {
    return '이';
  } else {
    return val;
...
// filter(callback) : 배열의 모든 요소에 대해 콜백함수를 반복 실행 후,
